              ...
              <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                <Typography variant="body2" color="text.secondary">
                  {samplePagePreviews.length > 0
                    ? `Đã chọn ${samplePagePreviews.length} ảnh trang đọc thử`
                    : 'Chưa chọn ảnh đọc thử'}
                </Typography>
              </Box>
              {samplePagePreviews.length > 0 && (
            ...
            <Grid item xs={12}>
              <TextField
                ...
                onChange={(e) => {
                  ...
                  const urlItems = e.target.value
                    .split(',')
                    .map((item) => item.trim())
                    .filter(Boolean)
                  const uploadItems = samplePagePreviews.filter((item) => item.startsWith('data:'))
                  setSamplePagePreviews([...urlItems, ...uploadItems].slice(0, 10))
                }}
